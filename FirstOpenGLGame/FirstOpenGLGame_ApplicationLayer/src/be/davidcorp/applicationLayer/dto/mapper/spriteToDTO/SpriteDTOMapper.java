...
public class SpriteDTOMapper {
	...
	public static SpriteDTO doAutoMappingForSprite(Sprite sprite) {
		...
		if (sprite instanceof Enemy) {
			spriteDTO = OrganicSpriteDTOMapper
					.mapEnemyToEnemyDTO((Enemy) sprite);
		}
		...
		if (sprite instanceof ConstructionSprite) {
			spriteDTO = ConstructionSpriteDTOMapper
					.mapConstructionSpriteToConstructionSpriteDTO((ConstructionSprite) sprite);
		}
		if (spriteDTO == null) {
			throw new MapperException("No mapping found for sprite: "
	...
	public static Sprite doAutoMappingForSpriteDTO(SpriteDTO spriteDTO) {
		...
		if (spriteDTO instanceof EnemyDTO) {
			sprite = OrganicSpriteDTOMapper
					.mapEnemyDTOToEnemy((EnemyDTO) spriteDTO);
		}
		if (spriteDTO instanceof LightDTO) {
			sprite = LightToLightDTOMapper
					.mapLightDTOToLight((LightDTO) spriteDTO);
		}
		if (spriteDTO instanceof ConstructionSpriteDTO) {
			sprite = ConstructionSpriteDTOMapper
					.mapConstructionSpriteDTOToConstructionSprite((ConstructionSpriteDTO) spriteDTO);
		}
		if (spriteDTO == null) {
			throw new MapperException("No mapping found for sprite: "
					+ spriteDTO);
	...
	public static void mapSpriteToSpriteDTO(SpriteDTO spriteDTO, Sprite sprite) {
		...
		if (sprite.getColor() != null) {
			ColorDTO colorDTO = new ColorDTO(sprite.getColor().getRed(), sprite
	...
	public static void mapSpriteDTOToSprite(Sprite sprite, SpriteDTO spriteDTO) {
		try {
			...
			sprite.getHitBox().setUpperRightPoint(
					spriteDTO.getUpperRightPoint());
			...
			if (spriteDTO.getColor() != null) {
				Color color = new Color(spriteDTO.getColor().getRed(),
						spriteDTO.getColor().getGreen(), spriteDTO.getColor()
								.getBlue());
